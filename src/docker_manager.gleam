...
fn serve_docker(request: Request(Connection)) -> Response(ResponseData) {
  mist.websocket(
    request:,
    on_init: init_counter_socket,
    handler: loop_docker_socket,
...
type DockerSocket {
  DockerSocket(
    component: lustre.Runtime(docker.Msg),
    self: Subject(server_component.ClientMessage(docker.Msg)),
  )
}

type DockerSocketMessage =
  server_component.ClientMessage(docker.Msg)

type DockerSocketInit =
  #(DockerSocket, Option(Selector(DockerSocketMessage)))

fn init_counter_socket(_) -> DockerSocketInit {
  let counter = docker.component()
  ...
  #(DockerSocket(component:, self:), Some(selector))
}

fn loop_docker_socket(
  state: DockerSocket,
  connection: mist.WebsocketConnection,
  message: mist.WebsocketMessage(DockerSocketMessage),
) {
  case message {
...
fn close_counter_socket(state: DockerSocket) -> Nil {
  server_component.deregister_subject(state.self)
  |> lustre.send(to: state.component)
}
